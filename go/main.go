...
func getTransactions(w http.ResponseWriter, r *http.Request) {
	...
	if itemID > 0 && createdAt > 0 {
		...
		// 1st page
		err := tx.Select(&items,
			"SELECT * FROM (SELECT * FROM `items` WHERE `seller_id` = ? UNION SELECT * FROM `items` WHERE  `buyer_id` = ?) AS t ORDER BY `created_at` DESC, `id` DESC LIMIT ?",
			user.ID,
			user.ID,
			TransactionsPerPage+1,
		)
	...
	for _, item := range items {
		...
		if val, ok := transactionEvidences[itemDetail.ID]; ok {
			...
			itemDetail.ShippingStatus = val.Shipping.Status
		}
...
func postShip(w http.ResponseWriter, r *http.Request) {
	...
	ok, err := isMemberOfItemIDs(itemID)
	if err != nil {
		log.Print(err)
		outputErrorMsg(w, http.StatusInternalServerError, "db error")
		tx.Rollback()
	...
	ok, err = isMemberOfTradingItemIDs(itemID)
	if err != nil {
		log.Print(err)
		outputErrorMsg(w, http.StatusInternalServerError, "db error")
		tx.Rollback()
